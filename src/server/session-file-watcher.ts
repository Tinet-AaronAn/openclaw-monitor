...
type SessionMessageCallback = (sessionId: string, message: string, sender: string, time: number) => void;
...
export class SessionFileWatcher {
  ...
  private async loadLastUserMessage(filePath: string, _sessionId: string, content?: string): Promise<void> {
    try {
      if (!content) {
  ...
  private handleUserMessage(messageData: any, filePath?: string): void {
    ...
    // 提取 session ID
    const sessionId = filePath ? filePath.split('/').pop()?.replace('.jsonl', '') : messageData.id;
    if (!sessionId) return;
    ...
    // 从消息文本中解析 sender（DingTalk 格式）
    const senderMatch = messageText.match(/"label"\s*:\s*"([^"]+)"/);
    if (senderMatch) {
    ...
    displayText = displayText
      ...
      .replace(/Conversation info \(untrusted metadata\):[\s\S]*?```json[\s\S]*?```[\s]*/g, '')
      // 移除 Sender 块
      .replace(/Sender \(untrusted metadata\):[\s\S]*?```json[\s\S]*?```[\s]*/g, '')
    ...
    console.log(`[SessionFileWatcher] User message from ${sender}: ${displayText.slice(0, 50)}...`);
  }
