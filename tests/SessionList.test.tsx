...
describe("SessionList Component", () => {
  ...
  it("should call onSelect when clicking a session", () => {
    ...
    const firstSessionButton = screen
      .getByText(/session-1-active/)
      .closest("button");
  ...
  it("should highlight selected session", () => {
    render(
      <SessionList
        sessions={mockSessions}
        runs={mockRuns}
        selectedSession="session-1-active"
        onSelect={mockOnSelect}
      />,
    );
  ...
  it("should truncate long session keys", () => {
    const longSession: Session = {
      sessionKey:
        "this-is-a-very-long-session-key-that-should-be-truncated-when-displayed",
    ...
    // 应该显示截断后的文本
    expect(
      screen.getByText(/this-is-a-very-long-session-key-th.../),
    ).toBeInTheDocument();
  });
});
