...
describe('EventCoordinator', () => {
  ...
  describe('handleToolCall', () => {
    it('should store tool call parameters', () => {
      coordinator.handleToolCall('call-123', 'read', { path: '/test/file.ts' }, 'read /test/file.ts');
    ...
    it('should process pending tool starts when tool call arrives late', () => {
      ...
      // 稍后收到参数
      coordinator.handleToolCall('call-late', 'exec', { command: 'ls -la' }, 'exec ls -la');
  ...
  describe('enrichEvent', () => {
    ...
    it('should not override existing args', () => {
      ...
      coordinator.handleToolCall('call-xyz', 'read', { path: '/other' }, 'other');
    ...
    it('should enrich tool event with matching toolCallId', () => {
      coordinator.handleToolCall('call-match', 'write', { path: '/file.ts', content: 'hello' }, 'write /file.ts');
      ...
      expect((result.data as any).rawArgs).toEqual({ path: '/file.ts', content: 'hello' });
    });
  });
  ...
  describe('cleanup', () => {
    it('should limit toolCallMap size to prevent memory leak', () => {
      ...
      for (let i = 0; i < 1100; i++) {
        coordinator.handleToolCall(`call-${i}`, 'read', { path: `/file-${i}` }, `read /file-${i}`);
      }
