...
describe("RunList Component", () => {
  ...
  it("should show only active runs by default", () => {
    render(
      <RunList runs={mockRuns} selectedRun={null} onSelect={mockOnSelect} />,
    );

    // 默认显示 active only，所以只有 running 状态的 run
    expect(screen.getByText("run-1-active".slice(-12))).toBeInTheDocument();
    expect(
      screen.queryByText("run-2-completed".slice(-12)),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("run-3-failed".slice(-12)),
    ).not.toBeInTheDocument();
  });
  ...
  it("should call onSelect when clicking a run", () => {
    ...
    const firstRunButton = screen
      .getByText("run-1-active".slice(-12))
      .closest("button");
    if (firstRunButton) {
  ...
  it("should highlight selected run", () => {
    render(
      <RunList
        runs={mockRuns}
        selectedRun="run-1-active"
        onSelect={mockOnSelect}
      />,
    );

    const selectedRun = screen
      .getByText("run-1-active".slice(-12))
      .closest("button");
